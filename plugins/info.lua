...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = '----'
    ...
    if not value then
	 if result.id == tonumber(💔ـــαπσπφmσμsـــ💔) then
	   text = text..'Rank💔 : 💔ـــαπσπφmσμsـــ💔 \n\n'
  ...
  local text = 'Full name : '..(result.from.first_name or '')..' '..(result.from.last_name or '')..'\n'
	...
	local hash = 'rank:'..result.to.id..':variables'
		local value = redis:hget(hash, result.from.id)
		 if not value then
		    if result.from.id == tonumber(💔ـــαπσπφmσμsـــ💔) then
		       text = text..'Rank💔 : 💔ـــαπσπφmσμsـــ💔 \n\n'
  ...
  if msg.from.username then
   ...
   local hash = 'rank:'..msg.to.id..':variables'
	if hash then
	  ...
	  if not value then
		if msg.from.id == tonumber(💔ـــαπσπφmσμsـــ💔) then
		 text = text..'Rank💔 : 💔ـــαπσπφmσμsـــ💔 \n\n'
